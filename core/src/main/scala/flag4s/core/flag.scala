...
trait FlagOps {
  ...
  def ifIs[A: Encoder, B](flag: Flag, value: A)(f: => B)(implicit store: Store): IO[Either[Throwable, B]] =
    is(flag, value).flatMap {
      case Right(true) => IO.pure(f.asRight)
      case _ => IO.pure(new RuntimeException(s"${flag.value} is not equal to ${value.asJson}").asLeft)
    }

  def is[A: Encoder](flag: Flag, value: A)(implicit store: Store): IO[Either[Throwable, Boolean]] = IO.pure((flag.value == value.asJson).asRight)
  ...
  def checkType[A: Encoder](flag: Flag, value: A): IO[Either[Throwable, Unit]] = {
    ...
    IO.pure(if (org == des) Right((): Unit) else new RuntimeException(s"type mismatch! can't replace $org with $des").asLeft)
  }
}
